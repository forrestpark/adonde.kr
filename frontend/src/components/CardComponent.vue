<template>
  <div>
    <div>
      <v-btn icon style="float: left" @click="[changeHeart(), storedMypage()]">
...
export default {
  ...
  computed: {
    ...mapState(["searchResults", "user", "userStoredCities"]),
  },
  data() {
    return {
      checkSido_sgg: "",
  ...
  methods: {
    ...mapMutations(["updateUserStoredCities"]),
    changeHeart() {
    ...
    async apiDeleteStored() {
      try {
        const cities = await axios.put(`${BASE_URL}/user/deleteStoredCity`, {
          id: this.user.id,
    ...
    async apiAddStored() {
      try {
        const cities = await axios.put(`${BASE_URL}/user/addStoredCity`, {
          id: this.user.id,
          sido_sgg: this.sido_sgg_value,
        });
