  ...
  <v-app>
      ...
      <div style="float: left; top: 20px">
        <v-list-item>
          <v-list-item-avatar>
            ...
            <img v-else :src="user.profile_image" />
          </v-list-item-avatar>
        ...
        <div style="position: absolute; bottom: 0">
          <v-list-item v-for="(icon, idx) in iconItems" :key="idx">
            <v-btn :href="icon.link" class="mx-4" icon>
              <v-icon size="24px">
                {{ icon.icon }}
              </v-icon>
            </v-btn>
    ...
    >
      ...
      <div class="languageBtn">
        <v-btn outlined width="100%" @click="modal = true">
    ...
    <v-main>
      <div
        class="MenuBar"
        style="position: relative; background-color: #a0dcb0"
      >
        <!-- <div 
      ...
      <v-list style=" background-color : #a0dcb0">
        ...
        <translate-modal @close="modal = false" v-if="modal">
          <h3 slot="header">
            언어를 고르세요
            <i class="has ha-times closeModalBtn" @click="modal = false"></i>
          </h3>
        </translate-modal>
    ...
    if (JSON.parse(sessionStorage.getItem("user")) != null) {
      ...
      if (this.$store.state.user == "") {
        ...
        this.$store.commit(
          ...
          JSON.parse(sessionStorage.getItem("user")).storedCities
        );
      }
      console.log("mypage disabled false");
      this.items[1].disabled = false;
